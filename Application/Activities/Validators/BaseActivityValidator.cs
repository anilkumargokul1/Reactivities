...
:BaseActivityDto
{
    public BaseActivityValidator(Func<T,TDto> selector)
    {
        ...
        .MaximumLength(100).WithMessage("Title is allowed to enter with in 100 charecters");
        RuleFor(x=>selector(x).Description).NotEmpty().WithMessage("Description is required");
        RuleFor(x=>selector(x).Date).GreaterThan(DateTime.Now).WithMessage("Date must be in the feature");
